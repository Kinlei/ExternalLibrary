...
for Enum, _ in next, EventEnums do
    Event.Events[Enum] = {};
end
...
function Event.Invoke(self, Enum, ObjID, ...)
    local Event = self.Events[Enum][ObjID];
    if Event then
        Event:Fire(...);
    end
end
...
function External.new(Class)
    ...
    if Supported[Class] then
        ...
        local function UpdateEdges(Pos, Size)
            Events.LeftSide = Pos.X;
            Events.RightSide = Pos.X + Size.X;
            Events.TopSide = Pos.Y;
            Events.BottomSide = Pos.Y + Size.Y;
